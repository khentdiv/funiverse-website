...
if ($check_table->num_rows == 0) {
    // Create orders table with product_id column
    ...
    $check_column = $conn->query("SHOW COLUMNS FROM orders LIKE 'product_id'");
    if ($check_column->num_rows == 0) {
        // Add product_id column if it doesn't exist
        $conn->query("ALTER TABLE orders ADD COLUMN product_id INT NULL AFTER user_id");
        $conn->query("ALTER TABLE orders ADD FOREIGN KEY (product_id) REFERENCES products(product_id) ON DELETE SET NULL");
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['place_order'])) {
    ...
    // Insert order
    $query = "INSERT INTO orders (user_id, product_id, order_number, quantity, total_amount, payment_method, payment_status, order_status, delivery_address, gcash_reference, created_at) 
              VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, NOW())";
    
    $stmt = $conn->prepare($query);
    $stmt->bind_param("iisidssss", 
        $_SESSION['user_id'], 
        $product_id, 
        $order_number,
        $quantity,
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            ...
            <p>Review your selected item and choose payment method</p>
        </div>
        ...
        <form method="POST" action="" id="checkout-form">
            <div class="checkout-grid">
                <!-- Left Column - Product Preview, Payment & Details -->
                <div class="checkout-card">
                    <!-- Product Image Preview -->
                    <div class="product-preview">
                        <div class="preview-title">
                            <i class="fas fa-couch"></i>
                            <span>Your selected item</span>
                        </div>
                        ...
                        <div class="image-container">
                            ...
                            <?php else: ?>
                                <i class="fas fa-chair" style="font-size: 5rem; color: #cfb087;"></i>
                            <?php endif; ?>
                        </div>
                        
                        <div class="quantity-badge">
                            <i class="fas fa-shopping-cart"></i> 
                            Quantity: <?php echo $quantity; ?> × ₱<?php echo number_format($product['base_price'], 2); ?>
                        </div>
                    </div>
                            ...
                            <div class="gcash-section" id="gcash-details">
                                <div class="gcash-details">
                                    <div class="gcash-qr">
                                        ...
                                        <p class="gcash-amount">₱<?php echo number_format($total_price, 2); ?></p>
                                    </div>
                        ...
                        <label class="payment-option" id="cod-option">
                            ...
                            <div class="gcash-section hidden" id="cod-details">
                                <div style="background: #1e192c; border: 1px solid #332d44; border-radius: 30px; padding: 1.5rem; text-align: center;">
                                    ...
                                    <p style="color: #b3a4cb;">Pay exactly <strong style="color: #cfb087;">₱<?php echo number_format($total_price, 2); ?></strong> when your order arrives</p>
                    ...
                    <div class="address-section">
                        ...
                        <textarea name="delivery_address" id="delivery_address" placeholder="Enter your complete delivery address" required><?php echo htmlspecialchars($user['address'] ?? ''); ?></textarea>
                        ...
                        <div class="order-item">
                            ...
                            <div class="item-details">
                                ...
                                <div class="item-quantity">
                                    <i class="fas fa-times"></i> <?php echo $quantity; ?> (₱<?php echo number_format($product['base_price'], 2); ?> each)
                                </div>
                            </div>
                            <div style="color: #cfb087; font-weight: 700;">₱<?php echo number_format($total_price, 2); ?></div>
                        </div>
                        <div class="summary-row total">
                            <span class="summary-label">Total</span>
                            <span class="summary-value total">₱<?php echo number_format($total_price, 2); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="action-buttons">
                        <button type="submit" name="place_order" class="btn btn-primary btn-large" id="place-order-btn">
                            <i class="fas fa-check-circle"></i> Place order (<?php echo $quantity; ?> item<?php echo $quantity > 1 ? 's' : ''; ?>)
                        </button>
                        <a href="collections.php" class="btn btn-large">
                ...
                <div class="order-summary" id="order-summary">
                    ...
                    <div class="summary-row">
                        <span class="summary-label">Subtotal (<?php echo $quantity; ?> item<?php echo $quantity > 1 ? 's' : ''; ?>)</span>
                    ...
                    <div style="margin-top: 2rem; background: #161224; border-radius: 30px; padding: 1.2rem;">
                        <p style="color: #b3a4cb; margin-bottom: 0.8rem; display: flex; align-items: center; gap: 0.5rem;">
                            <i class="fas fa-info-circle" style="color: #cfb087;"></i>
                            <strong style="color: #f0e6d2;">Product details:</strong>
                        </p>
                        <p style="color: #b3a4cb; font-size: 0.9rem; margin-bottom: 0.5rem;">
                            <?php echo htmlspecialchars($product['description']); ?>
                        </p>
                        <?php if (!empty($product['dimensions'])): ?>
                        <p style="color: #8e7daa; font-size: 0.85rem; margin-top: 0.5rem;">
                            <i class="fas fa-ruler"></i> Dimensions: <?php echo htmlspecialchars($product['dimensions']); ?>
                        </p>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        ...
        // Address auto-fill
        const defaultAddress = `<?php echo addslashes($user['address'] ?? ''); ?>`;
